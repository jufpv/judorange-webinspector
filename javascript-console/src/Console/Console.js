...
export default class Console extends Tool {
  ...
  _bindEvent() {
    ...
    $inputBtns
      ...
      .on('click', '.eruda-cancel', () => {
        $input.val('')
      })
      .on('click', '.eruda-previous', () => {
        $input.val(self._previous_command)
      })
      .on('click', '.eruda-execute', () => {
        let jsInput = $input.val().trim()
        if (jsInput === '') return
        self._previous_command = jsInput
        logger.input(jsInput) // Save commande
        $input
          .val('')
          .get(0)
          .blur()
    ...
    this._$el.on('click', '.eruda-expand-side-right', function(){
        let side_left = self._$el.find('.eruda-console-side-left').get(0)
